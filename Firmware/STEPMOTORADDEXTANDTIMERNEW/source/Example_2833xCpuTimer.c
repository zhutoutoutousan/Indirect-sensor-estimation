//###########################################################################
// Description
// This program is dedicated to realize the following functions:
// 1. Receives signal from Raspberry Model 3B GPIO1-4 which respectively repr
// -esents START,ACCELERATE,DECELERATE,STOP signal. When the signal comes, th
// -e processor controls step motor to perform start to nominal rpm, accelera
// -te by 1rpm, decelerate by 1rpm, stop to zero rpm.
//
//
// This program plans to use the following methods to realize the functions a
// -bove:
// 1. When the signal comes, it goes to Cputimer
//###########################################################################
...
// Enable TINT0 in the PIE: Group 1 interrupt 7
   ...
   EALLOW;
   GpioIntRegs.GPIOXINT3SEL.bit.GPIOSEL = 0x1c;   //select GPIO60 pin as XINT3 source
...
__interrupt void xint3_isr(void)
{
	...
	ROTFLAG = ~ ROTFLAG;
	DELAY_US(150);
	if(ROTFLAG == 65535)START_CON = START;
	if(ROTFLAG == 0x0 ) START_CON = STOP;

	/*  This is for two buttons
	if(CHZ<=400)
	   {
		   CHZ+=1;
...
__interrupt void xint4_isr(void)
{
	START_CON = START;
	DELAY_US(150);
	if(GpioDataRegs.GPBDAT.bit.GPIO61 == 0)START_CON = START;
	DELAY_US(5000000);
	if(GpioDataRegs.GPBDAT.bit.GPIO61 == 1 ) START_CON = STOP;
