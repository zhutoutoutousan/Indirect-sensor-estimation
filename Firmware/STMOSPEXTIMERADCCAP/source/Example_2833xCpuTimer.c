...
static float RCPSTRSP = RCP_STR_SMTSPEED;
static float CPHLM = CRCP_Hull_PRDNUM;
static float CSU = CRCP_STEP_uANG;
static Uint16 PS = PWM_PRESCALE ;
static Uint16 CD = CCALMD;
...
__interrupt void xint3_isr(void)
{
	START_CON = START;
	DELAY_US(150);
	if(GpioDataRegs.GPBDAT.bit.GPIO60 == 0)START_CON = START;
	DELAY_US(3000000);
	if(GpioDataRegs.GPBDAT.bit.GPIO60 == 0 ) START_CON = STOP;
...
__interrupt void xint4_isr(void)
{
	if(CHZ>5)
		   ...
		   {
			   EPwm1Regs.TBPRD = RPM2EPR(1.0f/6, CPHLM, CSU, CD);
		   }
	Xint4Count++;
...
*/
	   ...
	   R_PT = R_CT;
	   R_CT = ECap1Regs.CAP1;
	   R_RPM[array_index] = (float)(150000000*60)/((float)((R_CT-R_PT)*68))*24.5f;
	   tmp = 0;
	   for(av=0;av<10;av++)
	   {
	      tmp = tmp + R_RPM[av];
